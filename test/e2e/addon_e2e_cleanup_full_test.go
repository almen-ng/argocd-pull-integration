		...
		It("should cleanup addon but preserve Application when placement is updated", func() {
			...
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "--context", cluster1Context,
					"get", "argocd", "argocd", "-n", argoCDNamespace)
				_, err := utils.Run(cmd)
				g.Expect(err).To(HaveOccurred(), "ArgoCD CR should be deleted on spoke")
			}, 3*time.Minute, 5*time.Second).Should(Succeed(), "ArgoCD CR should be removed from spoke cluster")

			By("verifying ArgoCD pods are deleted on spoke cluster")
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "--context", cluster1Context,
					"get", "pods", "-n", argoCDNamespace,
				...
				output, err := utils.Run(cmd)
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(output).To(BeEmpty(), "ArgoCD namespace should have no pods")
			}, 2*time.Minute, 5*time.Second).Should(Succeed(), "All ArgoCD pods should be removed from spoke cluster")
